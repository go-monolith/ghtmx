...
func TestParse(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			path := filepath.Join(t.TempDir(), ".ghtmxignore")
...
func TestMatches(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			patterns: Patterns{"generator/test-*"},
			path:     "generator/test-foo/bar.ghtmx",
			expected: true,
		},
		{
			name:     "glob pattern does not match unrelated paths",
			patterns: Patterns{"generator/test-*"},
			path:     "generator/real-code/bar.ghtmx",
			expected: false,
		},
		{
...
func TestShouldSkipFunc(t *testing.T) {
	t.Run("missing file returns a function that never skips", func(t *testing.T) {
		skip, err := ShouldSkipFunc(t.TempDir(), ".ghtmxignore_fmt")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if skip("anything") {
	...
	t.Run("existing file returns a function that matches patterns", func(t *testing.T) {
		dir := t.TempDir()
		if err := os.WriteFile(filepath.Join(dir, ".ghtmxignore_fmt"), []byte("generator/test-*\n"), 0644); err != nil {
			t.Fatalf("failed to write test file: %v", err)
		}
		skip, err := ShouldSkipFunc(dir, ".ghtmxignore_fmt")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
