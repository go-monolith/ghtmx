...
func (rw *RangeWriter) closeLiteral(indent int) (r parser.Range, err error) {
	...
	sb.WriteString(`ghtmx_7f3b9d1a_Err = ghtmxruntime.WriteString(ghtmx_7f3b9d1a_Buffer, `)
...
func (rw *RangeWriter) writeErrorHandler(indentLevel int) (err error) {
	_, err = rw.WriteIndent(indentLevel, "if ghtmx_7f3b9d1a_Err != nil {\n")
	if err != nil {
		return err
	}
	indentLevel++
	_, err = rw.WriteIndent(indentLevel, "return ghtmx_7f3b9d1a_Err\n")
	if err != nil {
		return err
	}
	indentLevel--
