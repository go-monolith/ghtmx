...
func Test(t *testing.T) {
	...
	t.Run("attribute expressions can return errors", func(t *testing.T) {
		...
		t.Run("the errors are templ errors", func(t *testing.T) {
			...
			if templateErr.FileName != `internal/generator/test-attribute-errors/template.ghtmx` {
				t.Errorf("expected error in `internal/generator/test-attribute-errors/template.ghtmx`, but got %v", templateErr.FileName)
			}
