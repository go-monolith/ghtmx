...
import "fmt"
import "github.com/go-monolith/ghtmx/cmd/ghtmx/testproject/css-classes"
...
`,
			assertions: func(t *testing.T, updated string) {
				...
				if count := strings.Count(updated, "github.com/go-monolith/ghtmx/cmd/ghtmx/testproject/css-classes"); count == 0 {
...
import "fmt"
import  cssclasses "github.com/go-monolith/ghtmx/cmd/ghtmx/testproject/css-classes"
...
`,
			assertions: func(t *testing.T, updated string) {
				if count := strings.Count(updated, "cssclasses \"github.com/go-monolith/ghtmx/cmd/ghtmx/testproject/css-classes\""); count != 1 {
					t.Errorf("expected named import to be retained, got %d instances of it", count)
				}
				if count := strings.Count(updated, "github.com/go-monolith/ghtmx/cmd/ghtmx/testproject/css-classes"); count != 1 {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			// Create test project.
			dir, err := testproject.Create("github.com/go-monolith/ghtmx/cmd/ghtmx/testproject")
			if err != nil {
			...
			// Load the templates.ghtmx file.
			filePath := path.Join(dir, "templates.ghtmx")
