...
var templateExpressionParser = parse.Func(func(pi *parse.Input) (r templateExpression, matched bool, err error) {
	...
	// templ (x []string) Test() {
	// becomes:
	// func (x []string) Test() ghtmx.Component {
...
const (
	unterminatedMissingCurly = `unterminated (missing closing '{\n') - to escape "for", "if", "switch" etc. with braces, e.g. '{ "for" }' - https://ghtmx.guide/syntax-and-usage/statements#ifswitchfor-within-text`
	unterminatedMissingEnd   = `missing end (expected '}') - https://ghtmx.guide/syntax-and-usage/statements#ifswitchfor-within-text`
)
...
var templateNodeParsers = []parse.Parser[Node]{
	...
	fragmentDeclaration,    // fragment Name(a, b, c) {}
