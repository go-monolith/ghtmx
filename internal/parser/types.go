...
// FragmentDeclaration is a ghtmx-native construct (FR-030): a named,
// parameterised region declared either inside a page template (rendering
// inline at its declaration site, with parameters bound by name from the
// enclosing scope) or at the top level of a file. The compiler emits two
// entry points per fragment: inline and standalone (FR-031).
//
...
type FragmentDeclaration struct {
	...
	Range Range
	// Expression holds the name and parameter list, e.g. "UserRow(u User)".
	Expression Expression
	// Name of the fragment.
	Name string
	// Children of the fragment body.
	Children []Node
	// TopLevel records whether the fragment was declared at file level
	// rather than nested in a page template.
	TopLevel bool
}

func (f *FragmentDeclaration) IsNode() bool             { return true }
func (f *FragmentDeclaration) IsTemplateFileNode() bool { return true }
func (f *FragmentDeclaration) ChildNodes() []Node {
	return f.Children
}

func (f *FragmentDeclaration) Write(w io.Writer, indent int) error {
	source := formatFunctionArguments(f.Expression.Value)
	if err := writeIndent(w, indent, "fragment ", string(source), " {\n"); err != nil {
		return err
	}
	if err := writeNodesIndented(w, indent+1, f.Children); err != nil {
		return err
	}
	return writeIndent(w, indent, "}")
}

func (f *FragmentDeclaration) Visit(v Visitor) error {
	return v.VisitFragmentDeclaration(f)
}

// EventDeclaration is a top-level server-driven event declaration
// (FR-037): `event UserCreated(id string, name string)`. It binds an
// HX-Trigger wire name to a typed payload; a declaration with no payload
// is valid. Events generate emission symbols in the central generated
// package and have no body and no output of their own.
type EventDeclaration struct {
	Range Range
	Name  string
	// Expression holds the declaration signature, e.g.
	// "UserCreated(id string, name string)".
	Expression Expression
	// Comment holds a trailing line comment, e.g. "// emitted on signup".
	Comment string
}

func (e *EventDeclaration) IsTemplateFileNode() bool { return true }

func (e *EventDeclaration) Write(w io.Writer, indent int) error {
	source := formatFunctionArguments(e.Expression.Value)
	if e.Comment != "" {
		return writeIndent(w, indent, "event ", string(source), " ", e.Comment)
	}
	return writeIndent(w, indent, "event ", string(source))
}

func (e *EventDeclaration) Visit(v Visitor) error {
	return v.VisitEventDeclaration(e)
}
