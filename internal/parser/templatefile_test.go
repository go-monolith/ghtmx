...
const y = ` + "`456`"
	...
	// https://github.com/go-monolith/ghtmx/issues/505
...
func TestDefaultPackageName(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input:    "/files/on/disk/header.ghtmx",
			expected: "disk",
		},
		{
			name:     "path that starts with numbers",
			input:    "/files/on/123disk/header.ghtmx",
			expected: "main",
		},
		{
			name:     "path that includes hyphens",
			input:    "/files/on/disk-drive/header.ghtmx",
			expected: "main",
		},
		{
			name:     "relative path",
