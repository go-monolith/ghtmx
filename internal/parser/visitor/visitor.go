...
import "github.com/go-monolith/ghtmx/internal/parser"
...
func New() *Visitor {
	...
	v.EventDeclaration = func(n *parser.EventDeclaration) error {
		return nil
	}
	v.FragmentDeclaration = func(n *parser.FragmentDeclaration) error {
		for _, child := range n.Children {
			if err := child.Visit(v); err != nil {
				return err
			}
...
type Visitor struct {
	...
	FragmentDeclaration      func(n *parser.FragmentDeclaration) error
	EventDeclaration         func(n *parser.EventDeclaration) error
...
func (v *Visitor) VisitFragmentDeclaration(n *parser.FragmentDeclaration) error {
	return v.FragmentDeclaration(n)
}

func (v *Visitor) VisitEventDeclaration(n *parser.EventDeclaration) error {
	return v.EventDeclaration(n)
}
