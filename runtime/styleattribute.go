...
import (
	...
	"github.com/go-monolith/ghtmx/internal/safehtml"
)
...
// - ghtmx.KeyValue[string, string] - A map of key/values where the key is the CSS property name and the value is the CSS property value.
// - ghtmx.KeyValue[string, ghtmx.SafeCSSProperty] - A map of key/values where the key is the CSS property name and the value is the CSS property value.
// - ghtmx.KeyValue[string, bool] - The bool determines whether the value should be included.
// - ghtmx.KeyValue[ghtmx.SafeCSS, bool] - The bool determines whether the value should be included.
// - func() (anyOfTheAboveTypes)
// - func() (anyOfTheAboveTypes, error)
// - []anyOfTheAboveTypes
//
// In the above, ghtmx.SafeCSS and ghtmx.SafeCSSProperty are types that are used to indicate that the value is safe to render as CSS without sanitization.
...
func sanitizeStyleAttributeValue(sb *strings.Builder, v any) error {
	...
	case map[string]ghtmx.SafeCSSProperty:
		return processSafeCSSPropertyMap(sb, v)

	case ghtmx.KeyValue[string, string]:
		return processStringKV(sb, v)

	case ghtmx.KeyValue[string, bool]:
		if v.Value {
			return processString(sb, v.Key)
		}
		return nil

	case ghtmx.KeyValue[ghtmx.SafeCSS, bool]:
		if v.Value {
...
func processSafeCSS(sb *strings.Builder, v ghtmx.SafeCSS) error {
	if v == "" {
		return nil
	}
...
// processSafeCSSPropertyMap processes a map[string]ghtmx.SafeCSSProperty.
func processSafeCSSPropertyMap(sb *strings.Builder, m map[string]ghtmx.SafeCSSProperty) error {
	for _, name := range slices.Sorted(maps.Keys(m)) {
...
// processStringKV processes a ghtmx.KeyValue[string, string].
func processStringKV(sb *strings.Builder, kv ghtmx.KeyValue[string, string]) error {
