// Package martini is the ghtmx adapter for the martini framework,
// providing opt-in automatic render-mode selection (FR-035). martini
// injects (http.ResponseWriter, *http.Request) into its handlers, so
// the adapter needs no framework import at all and delegates to the
// nethttp adapter — mode selection, status, and header behavior are
// identical by construction; see that package for the documented
// defaults and overrides. The package exists so a martini application
// needs one adapter import, and no core package depends on it
// (constitution A5).
//
// martini hands handlers its own ResponseWriter wrapper; it is still a
// plain http.ResponseWriter that passes headers, status, and body
// straight through, so Render behaves exactly as under net/http.
// martini itself has been archived and unmaintained since 2017; the
// adapter exists for completeness of the framework coverage.
package martini
