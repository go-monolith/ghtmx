// Package fiberv3 is the ghtmx adapter for the fiber framework's v3
// major version, providing opt-in automatic render-mode selection
// (FR-035). fiber runs on fasthttp, so the adapter bridges its context
// to the net/http shapes the nethttp adapter works against and delegates
// to it — mode selection, status, and header behavior are identical by
// construction; see that package for the documented defaults and
// overrides. No core package depends on it (constitution A5).
//
// It differs from adapters/fiber only where v3 changed the surface:
// fiber.Ctx is an interface rather than *fiber.Ctx, and the
// request-scoped context accessor is Context() (v2's UserContext). The
// directory is fiberv3, not fiber/v3: Go would read a trailing /v3 as a
// module major-version suffix and demand v3.x.x tags, breaking the
// adapters' lockstep vX.Y.Z releases.
package fiberv3
...
func bridgeRequest(c fiberfw.Ctx) *http.Request {
	...
	// v3's Ctx.Context() is the request-scoped context (v2 called it
	// UserContext); it is context.Background() unless middleware set one —
	// unlike r.Context() on net/http, it does not cancel when the client
	// disconnects.
	return r.WithContext(c.Context())
}
