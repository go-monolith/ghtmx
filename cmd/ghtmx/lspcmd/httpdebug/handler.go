...
func getMapURL(uri string) ghtmx.SafeURL {
	return withQuery("/", uri)
}

func getSourceMapURL(uri string) ghtmx.SafeURL {
	return withQuery("/sourcemap", uri)
}

func getTemplURL(uri string) ghtmx.SafeURL {
	return withQuery("/templ", uri)
}

func getGoURL(uri string) ghtmx.SafeURL {
	return withQuery("/go", uri)
}

func withQuery(path, uri string) ghtmx.SafeURL {
	q := make(url.Values)
	q.Set("uri", uri)
	u := &url.URL{
		Path:     path,
