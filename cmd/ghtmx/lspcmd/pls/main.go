...
type Options struct {
	...
	RPCTrace bool
	Remote   string
	// OnExit fires when the gopls process ends (FR-085 supervision).
	OnExit func()
}
...
// NewGopls starts gopls and opens up a jsonrpc2 connection to it. OnExit,
// when set on opts, fires once the gopls process ends — the supervision
// hook for restart-with-backoff (FR-085).
func NewGopls(ctx context.Context, log *slog.Logger, opts Options) (location string, rwc io.ReadWriteCloser, err error) {
	...
	rwc, err = newProcessReadWriteCloser(log, cmd, opts.OnExit)
...
func newProcessReadWriteCloser(logger *slog.Logger, cmd *exec.Cmd, onExit func()) (rwc processReadWriteCloser, err error) {
