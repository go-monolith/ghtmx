...
func TestApplyEditConvertsWorkspaceEdit(t *testing.T) {
	...
	// The _ghtmx.go document change should be converted.
	if mock.applyEditParams.Edit.DocumentChanges[0].TextDocument.URI != "file:///project/component.ghtmx" {
		t.Errorf("expected _ghtmx.go URI to be converted, got %q", mock.applyEditParams.Edit.DocumentChanges[0].TextDocument.URI)
	}
...
func TestPublishDiagnosticsConvertsTemplGoFiles(t *testing.T) {
	...
	if mock.publishDiagnosticParams.URI != "file:///project/component.ghtmx" {
		t.Errorf("expected URI to be converted to .ghtmx, got %q", mock.publishDiagnosticParams.URI)
	}
...
// TestPublishDiagnosticsDropsUnmappedPositions: FR-085 — a gopls
// diagnostic whose position has no source-map entry is dropped, never
// reported at a wrong location.
func TestPublishDiagnosticsDropsUnmappedPositions(t *testing.T) {
	mock := &mockClient{}
	cache := NewSourceMapCache()
	cache.Set("file:///project/component.ghtmx", newTestSourceMap())
	client := Client{
	...
	err := client.PublishDiagnostics(context.Background(), &lsp.PublishDiagnosticsParams{
		...
		Diagnostics: []lsp.Diagnostic{
			{
				// Far outside anything the test source map covers.
				Range:   lsp.Range{Start: lsp.Position{Line: 9999, Character: 0}, End: lsp.Position{Line: 9999, Character: 5}},
				Message: "generated-only position",
			},
		},
	})
	if err != nil {
	...
	if mock.publishDiagnosticParams == nil {
		t.Fatal("the publish itself still forwards (possibly empty)")
	}
	if got := len(mock.publishDiagnosticParams.Diagnostics); got != 0 {
		t.Errorf("unmapped diagnostics must be dropped, got %d: %+v", got, mock.publishDiagnosticParams.Diagnostics)
	}
}
