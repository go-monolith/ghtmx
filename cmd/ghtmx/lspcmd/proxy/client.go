...
// file name from `*_ghtmx.go` to `*.ghtmx`, and to remap the char
...
func (p Client) PublishDiagnostics(ctx context.Context, params *lsp.PublishDiagnosticsParams) (err error) {
	...
	params.URI = templURI
	// Rewrite the positions. A position with no source-map entry belongs
	// to generated-only code: the diagnostic is dropped rather than
	// reported at a wrong location (FR-085).
	kept := make([]lsp.Diagnostic, 0, len(params.Diagnostics))
	for i, item := range params.Diagnostics {
		...
		if !ok {
			p.Log.Debug("dropping diagnostic at unmapped position", slog.Any("diagnostic", item))
			continue
		}
	...
	params.Diagnostics = p.DiagnosticCache.AddTemplDiagnostics(string(templURI), kept)
