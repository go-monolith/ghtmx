...
func TestDefinitionFromGoFile(t *testing.T) {
	mock := &mockServer{
		definitionResult: []lsp.Location{
			{URI: "file:///project/component_ghtmx.go", Range: goRange()},
	...
	// The _ghtmx.go result should be converted to .ghtmx.
	if result[0].URI != "file:///project/component.ghtmx" {
		t.Errorf("expected _ghtmx.go URI to be converted, got %q", result[0].URI)
	}
...
func TestDefinitionFromTemplFile(t *testing.T) {
	...
	if mock.definitionParams.TextDocument.URI != "file:///project/component_ghtmx.go" {
...
func TestRenameFromGoFile(t *testing.T) {
			...
			DocumentChanges: []lsp.TextDocumentEdit{
				{
					TextDocument: lsp.OptionalVersionedTextDocumentIdentifier{
						TextDocumentIdentifier: lsp.TextDocumentIdentifier{URI: "file:///project/component_ghtmx.go"},
					},
	...
	// The _ghtmx.go document change should be converted to .ghtmx.
	if result.DocumentChanges[0].TextDocument.URI != "file:///project/component.ghtmx" {
		t.Errorf("expected _ghtmx.go URI to be converted, got %q", result.DocumentChanges[0].TextDocument.URI)
	}
...
func TestRenameFromTemplFile(t *testing.T) {
	mock := &mockServer{
		renameResult: &lsp.WorkspaceEdit{
			Changes: map[lsp.DocumentURI][]lsp.TextEdit{
				"file:///project/component_ghtmx.go": {{Range: goRange(), NewText: "newName"}},
			},
		},
	}
	s := newTestServer(mock)
	...
	// The request should be rewritten to the Go file.
	if mock.renameParams.TextDocument.URI != "file:///project/component_ghtmx.go" {
		t.Errorf("expected URI to be converted to Go, got %q", mock.renameParams.TextDocument.URI)
	}
	// The response should be converted.
	if _, exists := result.Changes["file:///project/component_ghtmx.go"]; exists {
		t.Error("expected _ghtmx.go key to be removed from Changes map")
	}
	edits, exists := result.Changes["file:///project/component.ghtmx"]
	if !exists {
		t.Fatal("expected .ghtmx key to be present in Changes map")
	}
...
func TestImplementationNotHardcodedToRequestURI(t *testing.T) {
	...
	// The _ghtmx.go result should be converted, not hardcoded to the request URI.
	if result[0].URI != "file:///project/component.ghtmx" {
		t.Errorf("expected _ghtmx.go result to be converted, got %q", result[0].URI)
	}
...
func TestReferencesFromGoFile(t *testing.T) {
	...
	// _ghtmx.go reference converted to .ghtmx.
	if result[0].URI != "file:///project/component.ghtmx" {
		t.Errorf("expected _ghtmx.go reference to be converted, got %q", result[0].URI)
	}
...
func TestPrepareCallHierarchyConvertsItems(t *testing.T) {
	mock := &mockServer{
		prepareCallHierarchyResult: []lsp.CallHierarchyItem{
			{
				Name:           "MyComponent",
				URI:            "file:///project/component_ghtmx.go",
				Range:          goRange(),
...
func TestIncomingCallsConvertsFromItems(t *testing.T) {
	...
	if result[0].From.URI != "file:///project/component.ghtmx" {
...
func TestOutgoingCallsConvertsToItems(t *testing.T) {
	...
	if result[0].To.URI != "file:///project/component.ghtmx" {
...
func TestSymbolsConvertsTemplGoLocations(t *testing.T) {
	mock := &mockServer{
		symbolsResult: []lsp.SymbolInformation{
			{
				Name:     "MyComponent",
				Location: lsp.Location{URI: "file:///project/component_ghtmx.go", Range: goRange()},
			},
			{
				Name:     "main",
	...
	if result[0].Location.URI != "file:///project/component.ghtmx" {
		t.Errorf("expected _ghtmx.go symbol location to be converted, got %q", result[0].Location.URI)
	}
...
func TestCodeActionConvertsWorkspaceEdit(t *testing.T) {
	...
	if result[0].Edit.DocumentChanges[0].TextDocument.URI != "file:///project/component.ghtmx" {
...
func TestInitializeAdvertisesGoFileSupport(t *testing.T) {
	...
	if !ok {
		t.Fatal("expected experimental.ghtmx to be map[string]any")
	}
	goFileSupport, ok := templ["goFileSupport"].(bool)
	if !ok || !goFileSupport {
		t.Errorf("expected experimental.ghtmx.goFileSupport to be true, got %v", templ["goFileSupport"])
	}
}

// TestGhtmxDiagnosticsSharedAST: the analyzer consumes an already-parsed
// AST — never a second parser — so diagnostics reflect the given tree
// even when no file or text matches it. Also pins the LSP conversion:
// 1-based analyzer positions to 0-based point ranges, severities, and
// severity-override filtering.
func TestGhtmxDiagnosticsSharedAST(t *testing.T) {
	tf, err := parser.ParseString("package x\n\ntempl page() {\n\t<div hx-pots=\"/x\"></div>\n}\n")
	if err != nil {
		t.Fatal(err)
	}
	tf.Filepath = "file:///virtual/never-on-disk.ghtmx"
	surface, err := htmxsurface.ForVersion("2.0.10")
	if err != nil {
		t.Fatal(err)
	}

	s := &Server{Log: slog.New(slog.NewTextHandler(io.Discard, nil)), surface: surface}
	diags := s.ghtmxDiagnostics(tf)
	if len(diags) != 1 {
		t.Fatalf("expected one diagnostic, got %+v", diags)
	}
	d := diags[0]
	if d.Code != "GHTMX-E0201" || d.Source != "ghtmx" || d.Severity != lsp.DiagnosticSeverityError {
		t.Errorf("diagnostic identity mismatch: %+v", d)
	}
	if d.Range.Start.Line != 3 || d.Range.Start.Character != 6 || d.Range.End != d.Range.Start {
		t.Errorf("point range at 0-based 3:6 expected, got %+v", d.Range)
	}

	// Severity overrides behave identically to the CLI: Off suppresses
	// only warning-class checks, never errors.
	s.severityOverrides = map[string]diag.Severity{"GHTMX-E0201": diag.Off}
	if diags := s.ghtmxDiagnostics(tf); len(diags) != 1 {
		t.Errorf("error-class checks cannot be silenced (CLI parity), got %+v", diags)
	}
}
