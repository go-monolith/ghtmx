...
func TestConvertTemplToGoURI(t *testing.T) {
	tests := []struct {
		...
		{name: "templ file", input: "file:///tmp/test.ghtmx", wantIsTempl: true, wantGoURI: "file:///tmp/test_ghtmx.go"},
		{name: "go file", input: "file:///tmp/test.go", wantIsTempl: false, wantGoURI: ""},
		{name: "templ go file", input: "file:///tmp/test_ghtmx.go", wantIsTempl: false, wantGoURI: ""},
	}
...
func TestConvertTemplGoToTemplURI(t *testing.T) {
	tests := []struct {
		...
		{name: "templ go file", input: "file:///tmp/test_ghtmx.go", wantIsTemplGo: true, wantTemplURI: "file:///tmp/test.ghtmx"},
		{name: "regular go file", input: "file:///tmp/test.go", wantIsTemplGo: false, wantTemplURI: ""},
		{name: "templ file", input: "file:///tmp/test.ghtmx", wantIsTemplGo: false, wantTemplURI: ""},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestConvertLocationResults(t *testing.T) {
	...
	cache.Set("file:///tmp/test.ghtmx", newTestSourceMap())
	log := slog.Default()
	...
	tests := []struct {
		...
		},
		{
			...
			input: []lsp.Location{
				...
				{URI: "file:///tmp/test_ghtmx.go", Range: lsp.Range{Start: lsp.Position{Line: 15, Character: 20}, End: lsp.Position{Line: 15, Character: 30}}},
			},
			expected: []lsp.Location{
				{URI: "file:///tmp/other.go", Range: lsp.Range{Start: lsp.Position{Line: 1, Character: 2}, End: lsp.Position{Line: 3, Character: 4}}},
				{URI: "file:///tmp/test.ghtmx", Range: lsp.Range{Start: lsp.Position{Line: 5, Character: 10}, End: lsp.Position{Line: 5, Character: 20}}},
			},
		},
	}
	for _, tt := range tests {
...
func TestConvertWorkspaceEdit(t *testing.T) {
			...
			DocumentChanges: []lsp.TextDocumentEdit{
				{
					TextDocument: lsp.OptionalVersionedTextDocumentIdentifier{
						TextDocumentIdentifier: lsp.TextDocumentIdentifier{URI: "file:///tmp/test_ghtmx.go"},
					},
					Edits: []lsp.TextEdit{
						{
							Range: lsp.Range{
	...
	t.Run("converts Changes map", func(t *testing.T) {
		...
		templEdits, exists := edit.Changes["file:///tmp/test.ghtmx"]
		if !exists {
