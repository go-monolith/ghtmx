...
import (
	...
	"github.com/go-monolith/ghtmx/cmd/ghtmx/generatecmd/sse"
...
func reloadScript(nonce string) *html.Node {
	script := &html.Node{
		...
		Data: "script",
		Attr: []html.Attribute{
			{Key: "src", Val: "/_ghtmx/reload/script.js"},
		},
	}
	if nonce != "" {
...
func (h *Handler) modifyResponse(r *http.Response) error {
	...
	if r.Header.Get("ghtmx-skip-modify") == "true" {
		log.Debug("Skipping response modification because ghtmx-skip-modify header is set")
		return nil
	}
...
func (p *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	// The /_ghtmx/reload namespace is reserved for the dev server: nothing
	// under it is ever forwarded upstream (FR-063).
	if r.URL.Path == "/_ghtmx/reload" || strings.HasPrefix(r.URL.Path, "/_ghtmx/reload/") {
		switch r.URL.Path {
		case "/_ghtmx/reload/script.js":
			// Provides a script that reloads the page.
			w.Header().Add("Content-Type", "text/javascript")
			if _, err := io.WriteString(w, script); err != nil {
				fmt.Printf("failed to write script: %v\n", err)
			}
		case "/_ghtmx/reload/events":
			switch r.Method {
			case http.MethodGet:
...
func (rt *roundTripper) setShouldSkipResponseModificationHeader(r *http.Request, resp *http.Response) {
	...
	resp.Header.Set("ghtmx-skip-modify", "true")
}
...
func NotifyProxy(host string, port int) error {
	urlStr := fmt.Sprintf("http://%s:%d/_ghtmx/reload/events", host, port)
