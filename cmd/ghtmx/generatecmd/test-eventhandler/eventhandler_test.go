...
`)
	file, err := os.CreateTemp("", "*.ghtmx")
	if err != nil {
		t.Fatalf("failed to create temp file: %v", err)
	}
	templFileName := file.Name()
	goFileName := strings.TrimSuffix(templFileName, ".ghtmx") + "_ghtmx.go"
	defer func() {
		_ = file.Close()
...
func TestErrorLocationMapping(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			rawFileName: "single_error.ghtmx.error",
			errorPositions: []token.Position{
		...
		},
		{
			...
			rawFileName: "multiple_errors.ghtmx.error",
			errorPositions: []token.Position{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			// The raw files cannot end in .ghtmx because they will cause the generator to fail. Instead,
			// we create a tmp file that ends in .ghtmx only for the duration of the test.
			...
			file, err := os.CreateTemp("", fmt.Sprintf("*%s.ghtmx", test.rawFileName))
			if err != nil {
