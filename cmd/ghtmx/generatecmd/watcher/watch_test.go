...
func TestWatchDebouncesDuplicates(t *testing.T) {
	...
	go func() {
		rw.w.Events <- fsnotify.Event{Name: "test.ghtmx"}
		rw.w.Events <- fsnotify.Event{Name: "test.ghtmx"}
	}()
	count := 0
...
func TestWatchDoesNotDebounceDifferentEvents(t *testing.T) {
	tests := []struct {
		...
		// Different files
		{fsnotify.Event{Name: "test.ghtmx"}, fsnotify.Event{Name: "test2.ghtmx"}},
		// Different operations
		{
			fsnotify.Event{Name: "test.ghtmx", Op: fsnotify.Create},
			fsnotify.Event{Name: "test.ghtmx", Op: fsnotify.Write},
		},
		// Different operations and files
		{
			fsnotify.Event{Name: "test.ghtmx", Op: fsnotify.Create},
			fsnotify.Event{Name: "test2.ghtmx", Op: fsnotify.Write},
		},
	}
...
func TestWatchIgnoresFilesMatchingIgnorePattern(t *testing.T) {
	...
	ignorePattern, err := regexp.Compile(`ignore\.ghtmx$`)
	if err != nil {
	...
	go func() {
		...
		rw.w.Events <- fsnotify.Event{Name: "file.ignore.ghtmx"}
		// This should pass
		rw.w.Events <- fsnotify.Event{Name: "file.keep.ghtmx"}
	}()
...
func TestIgnorePatternTakesPrecedenceOverWatchPattern(t *testing.T) {
	...
	events := make(chan fsnotify.Event, 2)
	errors := make(chan error)
	watchPattern := regexp.MustCompile(`.*\.ghtmx$`)
	ignorePattern := regexp.MustCompile(`.*\.ignore\.ghtmx$`)
