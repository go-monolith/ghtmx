...
func TestFormat(t *testing.T) {
	...
	t.Run("can format a single file from stdin to stdout", func(t *testing.T) {
		...
		stdin := strings.NewReader(tp.testFiles["a.ghtmx"].input)
		...
		if diff := cmp.Diff(tp.testFiles["a.ghtmx"].expected, stdout.String()); diff != "" {
			t.Error(diff)
		}
	})
	...
	t.Run("can process a single file in place", func(t *testing.T) {
		...
		data, err := os.ReadFile(tp.testFiles["a.ghtmx"].name)
		if err != nil {
			t.Fatalf("failed to read file: %v", err)
		}
		if diff := cmp.Diff(tp.testFiles["a.ghtmx"].expected, string(data)); diff != "" {
			t.Error(diff)
		}
	})
	...
	t.Run("passes when fail flag used and no change occurs", func(t *testing.T) {
		...
		if err = Run(log, nil, nil, Arguments{
			Files: []string{
				tp.testFiles["c.ghtmx"].name,
			},
			FailIfChanged: true,
		}); err != nil {
			t.Fatalf("failed to run format command: %v", err)
		}
		data, err := os.ReadFile(tp.testFiles["c.ghtmx"].name)
		if err != nil {
			t.Fatalf("failed to read file: %v", err)
		}
		if diff := cmp.Diff(tp.testFiles["c.ghtmx"].expected, string(data)); diff != "" {
			t.Error(diff)
		}
	})
}
