...
func TestHandler(t *testing.T) {
	...
	errorComponent := ghtmx.ComponentFunc(func(ctx context.Context, w io.Writer) error {
		if _, err := io.WriteString(w, "Hello"); err != nil {
			t.Fatalf("failed to write string: %v", err)
		}
	...
	tests := []struct {
		...
		{
			name:             "handlers return OK by default",
			input:            ghtmx.Handler(ghtmx.Raw(`♠ ‘ &spades; &#8216;`)),
			expectedStatus:   http.StatusOK,
			expectedMIMEType: "text/html; charset=utf-8",
			expectedBody:     "♠ ‘ &spades; &#8216;",
		},
		{
			...
			input:            ghtmx.Handler(hello, ghtmx.WithStatus(http.StatusNotFound)),
		...
		},
		{
			...
			input:            ghtmx.Handler(hello, ghtmx.WithStatus(http.StatusOK), ghtmx.WithContentType("text/csv")),
			expectedStatus:   http.StatusOK,
		...
		},
		{
			...
			input:            ghtmx.Handler(errorComponent),
			expectedStatus:   http.StatusInternalServerError,
			expectedMIMEType: "text/plain; charset=utf-8",
			expectedBody:     "ghtmx: failed to render template\n",
		},
		{
			name: "error handling can be customised",
			input: ghtmx.Handler(errorComponent, ghtmx.WithErrorHandler(func(r *http.Request, err error) http.Handler {
	...
	t.Run("streaming mode allows responses to be flushed", func(t *testing.T) {
		...
		component := ghtmx.ComponentFunc(func(ctx context.Context, w io.Writer) error {
			// Write part 1.
		...
		ghtmx.Handler(component, ghtmx.WithStatus(http.StatusCreated), ghtmx.WithStreaming()).ServeHTTP(w, r)
	...
	t.Run("streaming mode handles errors", func(t *testing.T) {
		...
		h := ghtmx.Handler(component,
			ghtmx.WithStatus(http.StatusCreated),
			ghtmx.WithStreaming(),
			ghtmx.WithErrorHandler(errorHandler),
		)
		h.ServeHTTP(w, r)
