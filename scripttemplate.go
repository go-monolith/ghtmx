...
type ComponentScript struct {
	...
	// It would render:
	//    __ghtmx_functionName_sha(&#34;some string&#34;,12345))
	//
	// This is can be injected into HTML attributes:
	//    <button onClick="__ghtmx_functionName_sha(&#34;some string&#34;,12345))">Click Me</button>
	Call string
	...
	//    <script>__ghtmx_functionName_sha("some string",12345))</script>
	CallInline string
}
...
func SafeScript(functionName string, params ...any) string {
	if !jsFunctionName.MatchString(functionName) {
		functionName = "__ghtmx_invalid_js_function_name"
	}
	sb := new(strings.Builder)
