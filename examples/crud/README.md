...
The MVP acceptance demonstration (spec task 67): a todo manager with
create, read, update (inline title edit and done-toggle), and delete —
...
| FR-037 — event contract | `event TodoCreated/TodoToggled/TodoDeleted` declarations; handlers emit only `ghtmxgen.Emit*`; the stats panel listens with `hx-trigger="todo-created from:body, …"` |
