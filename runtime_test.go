...
func TestCSSID(t *testing.T) {
	t.Run("minimum hash suffix length is 8", func(t *testing.T) {
		...
		actual := len(ghtmx.CSSID(name, css))
	...
	t.Run("known hash collisions are avoided", func(t *testing.T) {
		...
		id1 := ghtmx.CSSID(name, css1)
		id2 := ghtmx.CSSID(name, css2)
		if id1 == id2 {
...
func TestCSSHandler(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:            []ghtmx.CSSClass{ghtmx.ComponentCSSClass{ID: "className", Class: ghtmx.SafeCSS(".className{background-color:white;}")}},
			expectedMIMEType: "text/css",
		...
		},
		{
			name: "classes are rendered",
			input: []ghtmx.CSSClass{
				ghtmx.ComponentCSSClass{ID: "classA", Class: ghtmx.SafeCSS(".classA{background-color:white;}")},
				ghtmx.ComponentCSSClass{ID: "classB", Class: ghtmx.SafeCSS(".classB{background-color:green;}")},
			},
			expectedMIMEType: "text/css",
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			h := ghtmx.NewCSSHandler(tt.input...)
...
func TestCSSMiddleware(t *testing.T) {
	...
	c1 := ghtmx.ComponentCSSClass{
		ID:    "c1",
		Class: ".c1{color:red}",
	}
	c2 := ghtmx.ComponentCSSClass{
		ID:    "c2",
		Class: ".c2{color:blue}",
	}
	...
	tests := []struct {
		...
		{
			name:             "accessing /style/ghtmx.css renders CSS, even if it's empty",
			input:            httptest.NewRequest("GET", "/styles/ghtmx.css", nil),
			handler:          ghtmx.NewCSSMiddleware(pageHandler),
			expectedMIMEType: "text/css",
			expectedBody:     "",
		},
		{
			name:             "accessing /style/ghtmx.css renders CSS that includes the classes",
			input:            httptest.NewRequest("GET", "/styles/ghtmx.css", nil),
			handler:          ghtmx.NewCSSMiddleware(pageHandler, c1, c2),
			expectedMIMEType: "text/css",
...
var cssInputs = []any{
	[]string{"a", "b"},          // []string
	"c",                         // string
	ghtmx.ConstantCSSClass("d"), // ConstantCSSClass
	ghtmx.ComponentCSSClass{ID: "e", Class: ".e{color:red}"}, // ComponentCSSClass
	map[string]bool{"f": true, "ff": false},                  // map[string]bool
	ghtmx.KV("g", true),                                      // KeyValue[string, bool]
	ghtmx.KV("gg", false),                                    // KeyValue[string, bool]
	[]ghtmx.KeyValue[string, bool]{
		ghtmx.KV("h", true),
		ghtmx.KV("hh", false),
	}, // []KeyValue[string, bool]
	ghtmx.KV(ghtmx.ConstantCSSClass("i"), true),   // KeyValue[CSSClass, bool]
	ghtmx.KV(ghtmx.ConstantCSSClass("ii"), false), // KeyValue[CSSClass, bool]
	ghtmx.KV(ghtmx.ComponentCSSClass{
		ID:    "j",
		Class: ".j{color:red}",
	...
	ghtmx.CSSClasses{ghtmx.ConstantCSSClass("k")},                // CSSClasses
	func() ghtmx.CSSClass { return ghtmx.ConstantCSSClass("l") }, // func() CSSClass
	ghtmx.CSSClass(ghtmx.ConstantCSSClass("m")),                  // CSSClass
	customClass{name: "n"},                                       // CSSClass
	[]ghtmx.CSSClass{customClass{name: "n"}},                     // []CSSClass
	ghtmx.KV(ghtmx.ConstantCSSClass("o"), true),                  // KeyValue[ConstantCSSClass, bool]
	[]ghtmx.KeyValue[ghtmx.ConstantCSSClass, bool]{
		ghtmx.KV(ghtmx.ConstantCSSClass("p"), true),
		ghtmx.KV(ghtmx.ConstantCSSClass("pp"), false),
...
func TestRenderCSS(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			toIgnore: []any{
				ghtmx.ComponentCSSClass{
					ID:    "c3",
					Class: ghtmx.SafeCSS(".c3{color:yellow}"),
				},
			},
			toRender: []any{c1, c2},
			expected: `<style type="text/css">.c1{color:red}.c2{color:blue}</style>`,
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err := ghtmx.RenderCSSItems(ctx, b, tt.toIgnore...)
			if err != nil {
			...
			b.Reset()
			err = ghtmx.RenderCSSItems(ctx, b, tt.toRender...)
			if err != nil {
...
func TestRenderCSSItemsWithNonce(t *testing.T) {
	ctx := ghtmx.WithNonce(context.Background(), "testnonce")
	b := new(bytes.Buffer)
	c := ghtmx.ComponentCSSClass{
		ID:    "c1",
		Class: ".c1{color:red}",
	}
	err := ghtmx.RenderCSSItems(ctx, b, c)
	if err != nil {
		t.Fatalf("failed to render CSS: %v", err)
	}
	actual := b.String()
...
func TestClassesFunction(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:    []any{"a", ghtmx.SafeClass("b"), ghtmx.Class("c")},
			expected: "a b c",
		},
		{
		...
		},
		{
			...
			input: []any{
				"a",
				map[ghtmx.CSSClass]bool{
					ghtmx.Class("b"): true,
					ghtmx.Class("a"): false,
					ghtmx.ComponentCSSClass{ID: "classC", Class: ghtmx.SafeCSS(".classC{color:red;}")}: true,
				},
			},
			expected: "b classC",
		},
		{
			name: "unknown types for classes get rendered as --ghtmx-css-class-unknown-type",
			input: []any{
				123,
			...
			expected: "--ghtmx-css-class-unknown-type c",
		},
		{
		...
		{
			name: "if a ghtmx.CSSClasses is passed in, the nested CSSClasses are extracted",
			input: []any{
				ghtmx.Classes(
					"a",
					ghtmx.SafeClass("b"),
		...
		},
		{
			...
			input: []any{
				...
				"c",
				ghtmx.KV("c", false),
				ghtmx.KV(ghtmx.SafeClass("d"), true),
				ghtmx.KV(ghtmx.SafeClass("e"), false),
			},
			expected: "a b d",
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			actual := ghtmx.Classes(test.input...).String()
...
func TestErrorWrapping(t *testing.T) {
	...
	wrappedErr := ghtmx.Error{Err: baseErr, Line: 1, Col: 2}
...
func TestRawComponent(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input:    ghtmx.Raw("<div>Test &</div>"),
			expected: `<div>Test &</div>`,
		},
		{
			name:        "Raw will return errors first",
			input:       ghtmx.Raw("", nil, errors.New("test error")),
			expected:    `<div>Test &</div>`,
			expectedErr: errors.New("test error"),
		},
		{
			name:     "Strings marked as safe are rendered without escaping",
			input:    ghtmx.Raw(template.HTML("<div>")),
			expected: `<div>`,
		},
	}
	for _, tt := range tests {
...
func TestGoHTMLComponents(t *testing.T) {
	t.Run("Go templates can be rendered as templ components", func(t *testing.T) {
		b := new(bytes.Buffer)
		err := ghtmx.FromGoHTML(goTemplate, "Test &").Render(context.Background(), b)
		if err != nil {
			t.Fatalf("failed to render content: %v", err)
		}
	...
	t.Run("templ components can be rendered in Go templates", func(t *testing.T) {
		b := new(bytes.Buffer)
		c := ghtmx.ComponentFunc(func(ctx context.Context, w io.Writer) (err error) {
			_, err = io.WriteString(w, "<div>Unsanitized &</div>")
			return err
		})
		h, err := ghtmx.ToGoHTML(context.Background(), c)
		if err != nil {
	...
	t.Run("errors in ToGoHTML are returned", func(t *testing.T) {
		...
		_, err := ghtmx.ToGoHTML(context.Background(), c)
		if err == nil {
	...
	t.Run("FromGoHTML does not require allocations", func(t *testing.T) {
		actualAllocs := testing.AllocsPerRun(4, func() {
			c := ghtmx.FromGoHTML(goTemplate, "test &")
			if c == nil {
				t.Fatalf("unexpected nil value")
			}
		})
...
func TestNonce(t *testing.T) {
	...
	t.Run("returns value if one has been set", func(t *testing.T) {
		expected := "abc123"
		ctx := ghtmx.WithNonce(context.Background(), expected)
		actual := ghtmx.GetNonce(ctx)
		if actual != expected {
			t.Errorf("expected %q got %q", expected, actual)
		}
...
func TestResolveAttributeValue(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			run:      func() (string, error) { return ghtmx.ResolveAttributeValue(`<script>alert("xss")</script>`) },
			expected: `&lt;script&gt;alert(&#34;xss&#34;)&lt;/script&gt;`,
		},
		{
			name:     "integer value is converted to string",
			run:      func() (string, error) { return ghtmx.ResolveAttributeValue(42) },
			expected: "42",
		},
		{
			name:     "bool value is converted to string",
			run:      func() (string, error) { return ghtmx.ResolveAttributeValue(true) },
			expected: "true",
		},
		{
			name: "ComponentScript returns pre-escaped Call field",
			run: func() (string, error) {
				return ghtmx.ResolveAttributeValue(ghtmx.ComponentScript{Call: `alert(&#34;Hello&#34;)`})
			},
			expected: `alert(&#34;Hello&#34;)`,
		},
		{
			name:    "error is returned",
			run:     func() (string, error) { return ghtmx.ResolveAttributeValue("value", errors.New("test error")) },
			wantErr: true,
		},
	}
	for _, tt := range tests {
...
func TestRenderAttributes(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			attributes: ghtmx.Attributes{
				"data-value":  ghtmx.KV("test-string", true),
				"data-hidden": ghtmx.KV("ignored", false),
			},
		...
		},
		{
			...
			attributes: ghtmx.Attributes{
				"checked":  ghtmx.KV(true, true),
				"disabled": ghtmx.KV(false, true),
				"hidden":   ghtmx.KV(true, false),
			},
			expected: ` checked`,
		},
		{
		...
		},
		{
			...
			attributes: ghtmx.Attributes{
				"data-name": ghtmx.KV("value", true),
				"active":    ghtmx.KV(true, true),
				"dynamic":   func() bool { return true },
				"ignored":   ghtmx.KV("ignored", false),
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := ghtmx.RenderAttributes(context.Background(), &buf, tt.attributes)
			if err != nil {
