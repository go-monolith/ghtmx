...
templ carve-outs, `W01xx` reachability, `W02xx` targets and inheritance,
`W03xx` build hygiene.
...
| `GHTMX-E0201` | An `hx-*` attribute name is unknown to the pinned htmx version — a misspelling, a suffixed attribute without its suffix (`hx-on`, `hx-status`), or an htmx 4 name modifier (`:inherited`, `:append`) on an attribute that does not inherit. | Fix the typo (the message suggests the nearest name), add the suffix, or drop the modifier. |
...
| `GHTMX-E0501` | An `hx-*` construct is not available at the pinned `htmxVersion`: introduced later (`:inherited` under htmx 2), or removed or renamed earlier (`hx-vars`, `hx-disabled-elt`, `hx-on::after-swap` under htmx 4). | Change `htmxVersion`, or follow the replacement the message names. |
...
| `GHTMX-W0202` | Under htmx 4 an inheritable attribute (`hx-target`, `hx-swap`, `hx-headers`, …) sits on an element that issues no request itself, without `:inherited`, while a descendant in the same template issues one — htmx 4 inheritance is explicit, so the descendant does not see it. `hx-headers` and `hx-boost` warn even without such a descendant (layouts render their children elsewhere). | Write `hx-target:inherited="…"`, move the attribute onto the requesting element, or silence with `GHTMX-W0202=off` (or `htmx.config.implicitInheritance`). |
