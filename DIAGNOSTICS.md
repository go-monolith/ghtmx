...
| `GHTMX-E0403` | A `//ghtmx:route` annotation is malformed. | Use `//ghtmx:route VERB /path <symbol>`, where the symbol is `Handler`, `pkg.Handler`, `Type.Method`, or `pkg.Type.Method` — a qualifier must be a package imported by the file or a type declared in this package. |
